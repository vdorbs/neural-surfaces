...
from torch import device, float64, no_grad, pi, save, sqrt, tensor, zeros, zeros_like
...
def train(rank):
    ...
    # Serve reconstruction renderings
    if rank == 0:
        render_fs = [[recon_fs.cpu() for recon_fs in all_recon_fs]]
        render_faces = [[manifold.faces.cpu() for manifold in all_manifolds]]
        render_uvs = [[zeros(manifold.num_vertices, 2) for manifold in all_manifolds]]
        serve_html(meshes_to_html(render_fs, render_faces, render_uvs, mode='none'), serve_locally=False)
